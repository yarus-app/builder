...
import importPKG from './utils/import-package-json';
...
async function build({ root: rootDir = process.cwd() } = {}) {
  ...
  const bundels = await Promise.all(options.map(({ input }) => rollup(input)));

  await Promise.all(bundels.map(({ write }, index) => write(options[index])));
}
